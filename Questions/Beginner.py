...
def occurance():
    ...
    most_frequent_num = l1[0]
    max_count = 0
    for i in l1:
        current_count = l1.count(i)
        # print(f"count of {i} is {current_count}")
        
        if current_count > max_count:
            max_count = current_count
            most_frequent_num = i
    print(f"The most frequent element is {most_frequent_num} which appears {max_count} times")
occurance()
...
# flatten means turn a nested list into a simple list
def flatten_list():
    a = [[1, 2], [3, 4], [5, 6]]
    l1 = []
    for i in a:
...
def second_smallest():
    ...
    print(unique_list)
    print(f"the second smallest element is : {unique_list[1]}")
second_smallest()
...
def square(n):
    a = n**0.5
    # square root of 25 is 5 and square root means (0.5)
    if n == a**2:
...
def rotate_list(k):
    a = [1,2,3,4,5]
    # k is position from we rotate the list
    k = k % len(a)
    # To handle cases where k is larger than list size
    return a[-2:] + a[:-2]
print(rotate_list(2))